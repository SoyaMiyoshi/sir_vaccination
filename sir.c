// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// print expected average payoff for conformist and imitator
...
int main(int argc, char *argv[]) {
	...
	float payoff_all = 0.0, payoff_zealot = 0.0, payoff_conformist = 0.0,
	      payoff_imitator = 0.0;
	int count_zealot = 0, count_conformist = 0, count_imitator = 0;

	// Calculate avg payoff for conformist, imitator, and all
	// Print Nan if does not exist
	// I know this is a bit inefficient as if 0 no if necessary
	for (i = 0; i < g.n; i++) {
		if (n[i].is_zealot) {
			payoff_zealot += n[i].payoff;
			count_zealot += 1;
		} else {
			if (n[i].is_conformist) {
				payoff_conformist += n[i].payoff;
				count_conformist += 1;
			} else {
				payoff_imitator += n[i].payoff;
				count_imitator += 1;
			}
		}
	}
	...
	printf("%f, %f, %f, %f\n",
	       payoff_zealot + payoff_conformist + payoff_imitator,
	       payoff_zealot / count_zealot,
	       payoff_conformist / count_conformist,
	       payoff_imitator / count_imitator);
	...
	for (i = 1; i < SEASONS; i++) {
		...
		fprintf(logfile, "First, agents make strategies "
				"depending upon their traits and neighbours'
		strategy\n\n");
		...
		*/
		...
		payoff_all = 0, payoff_zealot = 0, payoff_conformist = 0,
		payoff_imitator = 0;
		count_zealot = 0, count_conformist = 0, count_imitator = 0;
		...
		for (unsigned int k = 0; k < g.n; k++) {
			if (n[k].is_zealot) {
				payoff_zealot += n[k].payoff;
				count_zealot += 1;
				//printf("Node %d's, %d th zealot, payoff is %f, now payoff_zealot %f,\n", k, count_zealot, n[k].payoff, payoff_zealot);
			} else {
				if (n[k].is_conformist) {
					payoff_conformist += n[k].payoff;
					count_conformist += 1;
					//printf("Node %d's, %d th conformist, payoff is %f, now payoff_conformist %f,\n", k, count_conformist, n[k].payoff, payoff_conformist);
				} else {
					payoff_imitator += n[k].payoff;
					count_imitator += 1;
				    //printf("Node %d's, %d th imitator, payoff is %f, now payoff_imitator %f,\n", k, count_imitator, n[k].payoff, payoff_imitator);
				}
			}
		}
		...
		printf(
		    "%f, %f, %f, %f\n",
		    (payoff_zealot + payoff_conformist + payoff_imitator) / g.n,
		    payoff_zealot / count_zealot,
		    payoff_conformist / count_conformist,
		    payoff_imitator / count_imitator);
		...
		/*
		fprintf(logfile, "As a result of %d times SIR simulations, \n",
		NAVG); for (j = 0; j < g.n; j++) { fprintf(logfile, "Agent %d's
		", j); fprintf(logfile, "immunity:%d, (avg)payoff:%f,
		prob_infection %f \n", n[j].immunity, n[j].payoff, n[j].ninf /
		(float)NAVG);
		}*/
	}
