...
//?? I am not sure about the distribution of the numbers generated by pcg_16()
int GetRandomInt(float trsh){
    float my_rand = pcg_16()*(1.0)/(1 + UINT16_MAX);
...
void sir () {
	...
	unsigned int is_immune;

	for(i = 0; i < g.n; i++){
	    is_immune = GetRandomInt(g.coverage);
	    if(is_immune == 1){
	        n[i].heap = I_OR_R;
